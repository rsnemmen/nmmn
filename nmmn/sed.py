...
def pionDecay(datafile, alpha=2.3, E0=None, proton_energy=None, photon_energy=None):
	"""
...
:param datafile: log file generated by the semi-analytical RIAF model, containing the black hole mass, electron number density and ion temperature
...
• Zabalza 2015
	...
	# ## reads files
	# 
	# folder where model datafiles are located




	cd ~/science/projects/adafjet/adaf/perl/run01/
	...
	r,ti,h,ne = loadtxt('out_01',unpack=True,usecols=(0,3,7,14))
	...
	proton_energy = logspace(0, 6, 50) * u.MeV
	...
	photon_energy = logspace(0,6,100)*u.MeV
	...
	for i in tqdm.tqdm(range(nshells)):
	...
	# Plot
	loglog(photon_energy,thesed)
	xlabel('Photon energy [{0}]'.format(photon_energy.unit.to_string('latex_inline')))
	ylabel('$E^2 dN/dE$ [{0}]'.format(thesed.unit.to_string('latex_inline')))
